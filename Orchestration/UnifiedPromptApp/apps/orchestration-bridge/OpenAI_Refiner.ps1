...
param(
    [string]$Input,            # Raw goal text or inline text
    [string]$PromptPath,       # Path to canonical prompt YAML (preferred)
    [string]$Manifest,         # Optional manifest file for telemetry updates
...
if ($PromptPath) {
    if (-not (Test-Path $PromptPath)) {
        throw "PromptPath '$PromptPath' not found."
    }
    $GoalText = Get-Content -Raw $PromptPath
} elseif ($Input -and (Test-Path $Input)) {
    $GoalText = Get-Content -Raw $Input
} elseif ($Input) {
    $GoalText = $Input
} else {
    throw "Provide either -PromptPath or -Input."
}
...
if ($Manifest) {
    try {
        $manifestPath = Resolve-Path $Manifest -ErrorAction Stop
        $manifestJson = Get-Content -Raw $manifestPath | ConvertFrom-Json
        $manifestJson.status = "completed"
        $manifestJson.refined_goal = $OutFile
        $manifestJson.completed_at = (Get-Date).ToString("o")
        $manifestJson | ConvertTo-Json -Depth 6 | Out-File -FilePath $manifestPath -Encoding UTF8
    } catch {
        Write-Warning "Failed to update manifest '$Manifest': $_"
    }
}
