    ...
    try {
        ...
        const nodes = g.append('g')
          ...
          .join('g')
            ...
            .attr('cursor', 'pointer')
            .on('click', (_, d) => onSelectTask(d.data as Task));
        ...
        nodes.append('rect')
          ...
          .attr('rx', 8)
          .attr('ry', 8)
          .attr('fill', d => statusClasses[(d.data as Task).status].fill)
          .attr('stroke', d => (d.data as Task).id === selectedTaskId ? '#6366f1' : statusClasses[(d.data as Task).status].stroke)
          .attr('stroke-width', d => (d.data as Task).id === selectedTaskId ? 4 : 2)
          .classed('animate-pulse', d => (d.data as Task).status === TaskStatus.RUNNING);
        ...
        const div = foreignObject.append('xhtml:div')
          ...
          `)
          .style('color', d => statusClasses[(d.data as Task).status].text);
        ...
        mainContent.append('xhtml:div')
          ...
          .text(d => (d.data as Task).agent.role);
        ...
        div.append('xhtml:div')
            ...
            .style('border-top', d => (d.data as Task).cost !== undefined ? `1px solid ${statusClasses[(d.data as Task).status].stroke}` : 'none')
            .style('margin-top', '5px')
            .style('width', '100%')
            .text(d => (d.data as Task).cost !== undefined ? `$${(d.data as Task).cost!.toFixed(6)}` : '');
            
        // Add artifact indicator icon to nodes with artifacts
        nodes.filter(d => (d.data as Task).artifacts.length > 0)
          .append('path')
          ...
          .attr('fill', 'none')
          .attr('stroke', d => statusClasses[(d.data as Task).status].text)
