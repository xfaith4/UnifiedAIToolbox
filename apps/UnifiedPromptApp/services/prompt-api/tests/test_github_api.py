...
These tests focus on API contract behavior and avoid requiring network access.
"""
...
class TestGitHubStatus:
    def test_status_endpoint_has_expected_shape(self):
...
class TestGitHubAuth:
    ...
    def test_verify_auth_success(self, mock_github_class):
        ...
        response = client.post("/github/auth/verify", json={"token": "test_token"})
        assert response.status_code == 200
        data = response.json()
...
class TestListAccessibleRepositories:
    ...
    def test_list_accessible_repos_success(self, mock_service_class):
        ...
        response = client.get("/github/repos", headers={"Authorization": "Bearer testtoken"})
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
...
class TestRepoIntake:
    ...
    def test_repo_intake_error(self, mock_service_class):
        ...
        with patch("github_api.RepoIntakeError", new=Err):
            ...
            response = client.post("/github/repos/intake", json={"repo_url": "https://example.com/repo", "run_id": "r2"})
            assert response.status_code == 400
...
class TestTaskGraph:
    ...
    @patch("github_api.GITHUB_AVAILABLE", True)
    def test_taskgraph_missing_intake_file(self):
        response = client.post(
            "/github/supervisor/taskgraph",
            json={"run_id": "__pytest_missing_run_id__", "user_goal": "Do work", "constraints": {}},
        )
        assert response.status_code == 400
...
class TestRepositoryClone:
    ...
    def test_clone_repo_success(self, mock_parse, mock_service_class):
        ...
        response = client.post("/github/repos/clone", json={"repo_url": "https://github.com/owner/repo"})
        assert response.status_code == 200
        data = response.json()
    ...
    @patch("github_api.GITHUB_AVAILABLE", False)
    def test_clone_repo_unavailable(self):
        response = client.post("/github/repos/clone", json={"repo_url": "https://github.com/owner/repo"})
        assert response.status_code == 503


class TestPullRequestsIssuesBranches:
    @patch("github_api.GITHUB_AVAILABLE", True)
    @patch("github.Github")
    def test_list_pull_requests(self, mock_github_class):
        ...
        response = client.get("/github/repos/owner/repo/pulls", headers={"Authorization": "Bearer test_token"})
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    ...
    def test_list_issues(self, mock_github_class):
        ...
        response = client.get("/github/repos/owner/repo/issues", headers={"Authorization": "Bearer test_token"})
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
        assert data[0]["number"] == 1
    ...
    def test_list_branches(self, mock_github_class):
        ...
        response = client.get("/github/repos/owner/repo/branches", headers={"Authorization": "Bearer test_token"})
        assert response.status_code == 200
        data = response.json()
