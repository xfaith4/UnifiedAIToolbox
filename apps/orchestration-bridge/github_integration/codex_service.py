...
class CodexSwarmService:
    ...
    async def execute_codex_run(
        ...
        try:
            return_code = None
            try:
                # Start PowerShell process (async path).
                ...
                # Fallback for event loops that do not support subprocess APIs.
                loop = asyncio.get_running_loop()
                queue: asyncio.Queue[str | None] = asyncio.Queue()
                result: Dict[str, object] = {"return_code": None, "error": None}
            ...
            if return_code == 0:
                ...
                run_info['error'] = f'Codex swarm failed with return code {return_code}'
                yield {
                    'run_id': run_id,
            ...
            logger.exception("Codex run %s failed: %s", run_id, err_text)
            run_info['status'] = CodexRunStatus.FAILED
            run_info['error'] = err_text
            run_info['end_time'] = datetime.now(timezone.utc).isoformat()
