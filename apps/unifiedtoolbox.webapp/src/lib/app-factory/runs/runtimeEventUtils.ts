...
export function normalizeRuntimeEvent(raw: unknown, fallbackRunId?: string): RunEvent {
  if (raw && typeof raw === 'object') {
    ...
    const stage = typeof record.stage === 'string' ? record.stage : typeof record.phase === 'string' ? record.phase : undefined
    const step = typeof record.step === 'string' ? record.step : undefined
    const message = String(record.message || record.msg || record.status || 'event')
