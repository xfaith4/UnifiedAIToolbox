...
function Get-Prompt {
    ...
    $prompts = foreach ($f in $items) {
        try {
            ...
            Write-Warning ("⚠️  Skipped invalid YAML {0}: {1}" -f $f.FullName, $_)
        }
    }
...
function Search-Prompt {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Query, [int]$Max = 50)
...
function Export-Prompt {
    ...
    if (-not $OutFile) {
        ...
        $OutFile = Join-Path $exportRoot ("prompts_{0}{1}" -f $ts,$ext)
    }
    ...
    switch ($Format) {
        ...
        'Markdown' {
            ...
            foreach ($p in $list) {
                ...
                $null = $sb.AppendLine("```")
                $null = $sb.AppendLine("$($p.system)")
                $null = $sb.AppendLine("```")
                $null = $sb.AppendLine("")
                $null = $sb.AppendLine("### User Template")
                $null = $sb.AppendLine("```")
                $null = $sb.AppendLine("$($p.user_template)")
                $null = $sb.AppendLine("```")
                $null = $sb.AppendLine("\n---\n")
            }
...
Export-ModuleMember -Function Get-Prompt, Search-Prompt, Export-Prompt
